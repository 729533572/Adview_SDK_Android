...
public class DownloadService extends Service {
	...
	private Handler updateHandler = new Handler() {
		public void handleMessage(Message msg) {
			...
			case 0:
				...
				DownloadService.this.updateNotificationManager
						.cancel(DownloadService.index);
			...
			case 1:
				...
				DownloadService.this.updateNotification.setLatestEventInfo(
						DownloadService.this, DownloadService.this.title,
						description, DownloadService.this.updatePendingIntent);
				DownloadService.this.updateNotificationManager.notify(
						DownloadService.index,
						DownloadService.this.updateNotification);
				DownloadService.this
	...
	private void createNotication(Intent intent) {
		...
		if ("mounted".equals(Environment.getExternalStorageState())) {
			updateDir = new File(Environment.getExternalStorageDirectory(),
					"Adview/download/");
			updateFile = new File(this.updateDir.getPath(), this.title + index
					+ ".apk");
		}
		...
		updateNotification.setLatestEventInfo(this, "正在下载应用", "0%",
				updatePendingIntent);
		...
		try {
			...
			do {
				...
				if ((downloadCount == 0)
					...
					downloadCount += 10;
					this.updateNotification.setLatestEventInfo(this, "正在下载应用",
	...
	class DownloadRunnable implements Runnable {
		...
		public void run() {
			...
			try {
				long downloadSize = DownloadService.this.downloadFile(
						DownloadService.this.path,
						DownloadService.this.updateFile);
				if (downloadSize > 0L) {
